...
The Telco customer churn dataset was retrieved from Kaggle and was originally posted to predict customer behaviour and thus to improve the companies ability to retain them \citep { blastchar_2018}. The dataset contains information about a fictional telcom company that provided home phone and Internet services to 7043 customers in California in Q3 \citep {telco customer churn (11.1.3+)}. Due to the fictional nature of the dataset, we expect that IBM was mindful of potential biases and that the number of significant biases will thus most likely be limited. The dataset does not contain any time variable and all the datapoints are from a fictional third quarter. Hence, seasonality is not expected to play any meaningful role and will be neglected.
...
The first step in our modelling approach is creating the splits based on stratified sampling with a $\frac{n_{train}}{n_{total}} = 75\%$ ratio. 
When classifying a dataset with imbalanced classes, stratified resampling is generally a good idea  (Kohavi et al., 1995, p. 7).

Additionally, 5-fold stratified cross validation was employed for hyperparameter tuning the various models, where the same 5 folds are given to all models.
Setting churn as the stratification variable will keep the proportion of churning and not churning customers in each training set equal to the proportion in the complete dataset.

Citation: Kohavi, R. (1995, August). A study of cross-validation and bootstrap for accuracy estimation and model selection. In Ijcai (Vol. 14, No. 2, pp. 1137-1145).
